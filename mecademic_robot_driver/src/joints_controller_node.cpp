...
receive the joint position command from a topic and update the corresponding
global var
*/
...
receive the joint position measure from a topic and update the corresponding
global var
*/
...
*/
bool srv_server_set_enabled_cb(std_srvs::SetBool::Request &request,
  ...
  if (request.data != enabled) {
    ...
    ROS_INFO("mecademic/joints_controller %s",
...
int main(int argc, char *argv[]) {
  ...
  // Subscribers
  ros::Subscriber sub_joint_position_command = nh_public.subscribe(
      "command/joint_position", 1, joints_position_command_cb);
  ros::Subscriber sub_joint_position_measure = nh_public.subscribe(
      "state/joint_position", 1, joints_position_measure_cb);
  ...
  // Service Servers
  ros::ServiceServer srv_server_activate = nh_public.advertiseService(
      "joints_controller/set_enable", srv_server_set_enabled_cb);
...
void control_publish() {
  for (int i = 0; i < NUM_JOINTS; i++)
    vel_command_msg.joints[i] =
        gains[i] * (joints_command[i] - joints_measure[i]);
  vel_command_msg.header.stamp = ros::Time::now();
